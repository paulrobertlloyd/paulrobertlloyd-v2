            ...
            switch($data->type){
                case 'photo':
                    if(empty($data->url) || empty($data->width) || empty($data->height)){
                        ...
                        $html = '<a href="'.$url.'"><img src="'.$this->escapeHTML($this->safeUrl($data->url)).'" alt="'.$this->escapeHTML($title).'" width="720" /></a>'."\n";
                    }
                    break;

                case 'video':
                    if ($data->provider_name == 'YouTube') {
                        if (preg_match('#(http://www.youtube.com)?/(v/([-|~_0-9A-Za-z]+)|watch\?v\=([-|~_0-9A-Za-z]+)&?.*?)#i',$url,$output)) {
                            echo '<div class="object youtube"><iframe width="680" src="http://www.youtube.com/embed/'.$output[4].'?showinfo=0&#38;rel=0&#38;autohide=1"></iframe></div>'."\n";
                        }
                        else {
                    ...
                    if ($data->provider_name == 'Vimeo') {
                        if (preg_match('#(http://vimeo.com)/([-|~_0-9A-Za-z]+)#i',$url,$output)) {
                            echo '<div class="object vimeo"><iframe width="680" src="http://player.vimeo.com/video/'.$output[2].'?color=1177bb&#38;title=0&#38;byline=0"></iframe></div>'."\n";
                        }
                        else {
                            $html = ( !empty($data->html) ) ? $data->html : false;
                        }
